---
title: Maintain your brain, Completers only, Primary Cognitive Composite Variable
...
This is the completers analysis.  In this analysis, only those who completed the 3-year follow up are included. 

Update: 3 July 2022

This is a preliminary analysis, and small changes to the data may be expected. (need to confirm that final changes have or have not been made.)

`adri_baseline` is now included as a covariate. `adri_baseline` is a variable that is the value of `sum_newadri_bl` if it is not missing, or `adri_total` if `sum_newadri_bl` is missing.  This results in a baseline ADRI variable that has no missing values.
...
if(!params$final){
  ...
  # save models
  save(fitted_mods,file = paste0("../../Results/baseline_sd/",params$response,"_completers"))
  
} else {
  load(file = paste0("../../Results/baseline_sd/",params$response,"_completers"))
}
...
if(!params$final){
  ...
  # save models
  write.csv(res_tab,file = paste0("../../Results/baseline_sd/",params$response,"_completers_table.csv"), row.names = FALSE)
  

  
} else {
  res_tab <- read.csv(file = paste0("../../Results/baseline_sd/",params$response,"_completers_table.csv"))
...
The data were restricted to those who completed the final measurement (Year 3). This means that in group 1, there were 2090 people, and group 2, there were 1484 people for a total of 3574.  If a participant was missing a value for Year 1 or 2, but returned for the Year 3 measurement, they were included in this analysis.

The outcome variable is the calculated composite score from the full Baseline sample; it was *not* recalculated using only the 3574 in this analysis.
