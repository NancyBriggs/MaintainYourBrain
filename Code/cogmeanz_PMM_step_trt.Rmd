...
subtitle: Pattern Mixture Model  Scenario 2
...
mi_baseline = mi_dat %>% 
  ...
  select(-c(rc_trial_time, cogmeanz_full)) 

# observed outcome data
mi_all_times = mi_dat %>% 
  select(id_mask, rc_trial_time, cogmeanz_full)
...
## Scenario 2: Constant intervention


People who did not return in the intervention group were people whose scores would have been lower over follow-up time points. This assumes that the imputed missing values are actually lower than the imputation model predicted.
...
pmm_imputed_data_loc <- pmm_imputed_data %>%
  mutate(cogmeanz_full = ifelse(na_flag == 1 & group_type_id == 2, 
...
res_tab <- data.frame(response = "cogmeansz_full_ppm3",
                      adjusted = "No",
...
ggsave(file = paste0(params$response,"_",params$data,"_pmm2_plot.png"), width = 12, height = 8, units = "cm")
...
write.csv(file = paste0(params$response,"_",params$data,"_pmm2_plotdata.csv"), row.names = FALSE)
...
for(delta in c(0, 0.1, 0.2, 0.3, 0.35, 0.4)) {
  ...
  anova_call <- aov_pmm$out$`1 ~~ 2`$result %>%  as.data.frame()
  table <- data.frame(response = paste0("cogmeansz_full_ppm3 ", delta) ,
                        adjusted = "No",
                        comparison = cond_sum$rc_trial_time_consec,
                        did = round(cond_sum$estimate, 3), 
...
write.csv(res_all, file = paste0(params$response,"_",params$data,"_pmm2_result.csv"), row.names = FALSE)
...
This is a sensitivity analysis using the pattern mixture model (Little, 1993), assuming a mixture of MAR and missing not at random (MNAR) mechanisms for the missing data. In this analysis, missing values in the intervention group in the imputed datasets created for the MAR analysis were recalculated, assuming each value was `r !params$mnar_coeff` *lower* than the imputed values.  That is:
