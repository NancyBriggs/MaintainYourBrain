---
title: Maintain your brain, MediCul
output: html_document
params:
  data : "diet"
  response : "MEDICULTotal"
...
- Secondary Outcome: MEDICUL Total
...
temp <- myb_dat %>%
  ...
  dplyr::select(MEDICULTotal)
baselinesd<-sd(temp$MEDICULTotal)
es<-round(res_tab[1,3] / sd(temp$MEDICULTotal),3)
esadj<-round(res_tab[2,3] / sd(temp$MEDICULTotal),3)
rm(temp)

```

NOTE: MEDICUL is not baseline-standardised, so these results can be intepreted as:

group_type_id=2 showed a greater change of 8.720 points (unadjusted) on the MEDICUL than group_type_id=1 from baseline to 3 years.
...
$$ES_{unadj} = \frac{8.720}{12.627} = 0.691$$




$$ES_{adj} = \frac{8.657}{12.627} = 0.686$$


So, the change from baseline to three years was `r es` baseline SDs higher in group 2 than group 1 (unadjusted).
