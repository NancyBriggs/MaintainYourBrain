...
- BMI: `rcal_mybBMI`, `cl2_bmi_fu1`, `cl2_bmi_fu2`, `cl2_bmi_fu3`
...
myb_dat %>%
  group_by(.dots = lapply(params$predictors, as.symbol)) %>% 
    filter(!is.na(bmi))  %>%
  summarize(mean=mean(bmi, na.rm=T), sd=sd(bmi, na.rm=T))
...
One person is in the Baseline Sample, but did not have any data at baseline for BMI calculation.

```{r}
myb_dat %>%
  dplyr::select(id_mask, rc_trial_time, bmi, rcal_mybBMI, rcal_total_energy) %>%
  filter(is.na(bmi) & rc_trial_time==1)
```
...
# pvalue for diff in diffs
dif1 <- c(-1,0,0,0,0,0,1,0)
dif2 <- c(0,-1,0,0,0,0,0,1)
p.value.for.did<-data.frame(contrast(group_time_ems, method = list(dif1 - dif2) ) )
